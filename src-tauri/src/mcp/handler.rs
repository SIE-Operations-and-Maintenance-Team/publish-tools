...
impl McpHandler {
    pub fn new(app_handle: tauri::AppHandle, audit_logger: Option<Arc<Mutex<AuditLogger>>>) -> Self {
        Self {
            app_handle,
            audit_logger,
        }
...
impl McpHandler {
    ...
    async fn file_exists(&self, Parameters(params): Parameters<FilePathParam>) -> Result<CallToolResult, ErrorData> {
        Self::to_result(file_module::exists(&params.path).await)
    }

    /// 读取文件内容
    #[tool(description = "读取文件内容（UTF-8 编码）")]
    async fn file_read(&self, Parameters(params): Parameters<FilePathParam>) -> Result<CallToolResult, ErrorData> {
        Self::to_result(file_module::read_content_to_file(&params.path).await)
    }

    /// 写入内容到文件
    #[tool(description = "将内容写入文件（会覆盖已存在的文件）")]
    async fn file_write(&self, Parameters(params): Parameters<FileWriteParam>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn file_list(&self, Parameters(params): Parameters<FilePathParam>) -> Result<CallToolResult, ErrorData> {
        Self::to_result(file_module::read_files(&params.path).await)
    }

    /// 删除文件或目录
    #[tool(description = "删除指定的文件或目录路径列表（目录会递归删除）")]
    async fn file_delete(&self, Parameters(params): Parameters<FileDeleteParam>) -> Result<CallToolResult, ErrorData> {
        ...
        self.audit(
            AuditEntry::new("file_delete", result_str)
                .with_paths(params.paths),
        );
        Self::to_result(result)
    }
    ...
    async fn file_compress(&self, Parameters(params): Parameters<FileCompressParam>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn parse_sln(&self, Parameters(params): Parameters<ParseSlnParam>) -> Result<CallToolResult, ErrorData> {
        Self::to_result(parse_sln_module::parse_sln_project(
            &params.module_name, &params.sln_file_path,
            params.is_new_version, &params.build_mode,
        ).await)
    }

    /// 查找 .csproj 程序集名称
    #[tool(description = "查找 .csproj 项目文件中的 AssemblyName（程序集名称）")]
    async fn find_assembly_name(&self, Parameters(params): Parameters<FindAssemblyNameParam>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn upgrade_module_version(&self, Parameters(params): Parameters<UpgradeModuleVersionParam>) -> Result<CallToolResult, ErrorData> {
        let result = wpf_upgrade_module::upgrade_module_version(&params.file_path, &params.module_name).await;
        let result_str = match &result {
            Ok(_) => "ok",
            Err(_) => "error",
        };
    ...
    async fn copy_dll_files(&self, Parameters(params): Parameters<CopyDllFilesParam>) -> Result<CallToolResult, ErrorData> {
        let result = file_module::copy_dll_files(
            &params.source_dir, &params.target_dir, params.del_destination,
        ).await;
        let result_str = match &result {
            Ok(_) => "ok",
            Err(_) => "error",
    ...
    async fn project_build(&self, Parameters(params): Parameters<ProjectBuildParam>) -> Result<CallToolResult, ErrorData> {
        let result = file_module::build_project_release(
            &params.project_file_path, &params.msbuild_path,
            params.is_rebuild, &params.build_mode,
        ).await;
        let result_str = match &result {
            Ok(_) => "ok",
            Err(_) => "error",
    ...
    async fn local_exec(&self, Parameters(params): Parameters<LocalExecParam>) -> Result<CallToolResult, ErrorData> {
        let result = file_module::execute_local_command(
            &params.command, params.args.clone(), params.retry_count, params.retry_interval_secs,
        ).await;
        let result_str = match &result {
            Ok(_) => "ok",
            Err(_) => "error",
    ...
    async fn local_exec_spawn(&self, Parameters(params): Parameters<LocalExecSpawnParam>) -> Result<CallToolResult, ErrorData> {
        let result = file_module::exec_local_command_spawn(&params.command, params.args.clone()).await;
        let result_str = match &result {
            Ok(_) => "ok",
            Err(_) => "error",
        };
    ...
    async fn project_list(&self, Parameters(params): Parameters<ProjectListParam>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn appconfig_list(&self, Parameters(params): Parameters<AppConfigListParam>) -> Result<CallToolResult, ErrorData> {
        let conn = crate::mcp::db::open_db(&self.app_handle).map_err(|e| ErrorData::internal_error(e, None))?;
    ...
    async fn backup_list(&self, Parameters(params): Parameters<BackupListParam>) -> Result<CallToolResult, ErrorData> {
        let conn = crate::mcp::db::open_db(&self.app_handle).map_err(|e| ErrorData::internal_error(e, None))?;
    ...
    async fn backup_create(&self, Parameters(params): Parameters<BackupCreateParam>) -> Result<CallToolResult, ErrorData> {
        let conn = crate::mcp::db::open_db(&self.app_handle).map_err(|e| ErrorData::internal_error(e, None))?;
        ...
        // 审计日志
        self.audit(
            AuditEntry::new("backup_create", "ok")
                .with_file_path(&format!("project_id={},project_name={}", params.project_id, params.project_name)),
        );
    ...
    async fn backup_restore_list(&self, Parameters(params): Parameters<RestoreListParam>) -> Result<CallToolResult, ErrorData> {
        let conn = crate::mcp::db::open_db(&self.app_handle).map_err(|e| ErrorData::internal_error(e, None))?;
