  ...
  >
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
        ...
        >
          <i
            class="smom-icon icon-webicon318 layout-navbars-tagsview-ul-li-smom-icon"
            v-if="isActive(v)"
          ></i>
          <SvgIcon
            :name="v.meta.icon"
...
const Contextmenu = defineAsyncComponent(
  () => import("@/layout/navBars/tagsView/contextmenu.vue")
);
...
const onTagsClick = (v: RouteItem, k: number) => {
  ...
  if (getThemeConfig.value.layout === "columns") {
    const item: RouteItem = routesList.value.find(
      (r: RouteItem) => r.path.indexOf(`/${v.path.split("/")[1]}`) > -1
    );
    !item.children
...
const transUrlParams = (v: RouteItem) => {
  ...
  if (v.meta?.isDynamic) {
     ...
     * 2、通过事件调用时，路由的 path 不是原始定义的路由格式，如：/params/dynamic/details/vue3/111/我是动态路由测试tagsViewName(非国际化)
     *
...
onBeforeMount(() => {
  ...
  mittBus.on<any>("onCurrentContextmenuClick", (data: RouteItem) => {
    // 通过方法点击关闭 tagsView
    data.isFnClick = true;
...
.layout-navbars-tagsview {
  ...
  .tags-style-five {
    ...
    .layout-navbars-tagsview-ul-li {
      ...
      margin: 0 -15px;
      .layout-icon-active,
      .layout-navbars-tagsview-ul-li-smom-icon,
