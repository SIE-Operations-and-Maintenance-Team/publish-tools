  ...
  <div class="step-preview">
    <el-descriptions title="待发布清单" :column="1" border size="small">
      <el-descriptions-item label="项目">
        <span v-if="projectName"
          ...
          <el-tag size="small" style="margin-left: 6px"
            >ID {{ store.draft.projectId }}</el-tag
          ></span
        >
        <span v-else style="color: var(--el-text-color-placeholder)">{{
          store.draft.projectId ? `ID ${store.draft.projectId}` : "未选择"
        }}</span>
      </el-descriptions-item>
      <el-descriptions-item label="代码源">
        <span v-if="sourceLabel">{{ sourceLabel }}</span>
        <span v-else style="color: var(--el-text-color-placeholder)"
          >未选择（Git / TFS 至少其一）</span
        >
      </el-descriptions-item>
      <el-descriptions-item label="服务器">
        <span v-if="serverNames.length"
          >{{ serverNames.join("、") }}
          <el-tag size="small" style="margin-left: 6px"
            >{{ serverNames.length }} 台</el-tag
          ></span
        >
        <span v-else style="color: var(--el-text-color-placeholder)"
          >未选择</span
      ...
      <el-descriptions-item label="应用配置">
        <span v-if="hasAppconfig"
          >已配置 · {{ envLabel }} · {{ buildModeLabel }}</span
        >
        <span v-else style="color: var(--el-text-color-placeholder)"
          >未配置</span
        >
      ...
      <el-descriptions-item label="备份">
        ...
        <span
          v-if="store.draft.publishOptions?.backupBasePath"
          style="margin-left: 8px; word-break: break-all"
          >路径：{{ store.draft.publishOptions.backupBasePath }}</span
        >
      </el-descriptions-item>
    </el-descriptions>
    ...
    <div ref="logRef" class="preview-log">
      ...
      <el-empty
        v-if="logs.length === 0"
        description="执行预检或发布后，详细日志将在此处实时回显"
        :image-size="56"
      />
    </div>
  </div>
...
const hasAppconfig = computed(
  () =>
    ...
    Object.keys(store.draft.appconfigDraft as any).length > 0
);
...
const buildModeLabel = computed(
  () => (store.draft.appconfigDraft as any)?.buildMode || "—"
);
...
const getPublishFiles = async (
  ...
  if (dllMode === "TFS") {
    if (!dllModeValue) {
      pushLog("未配置 TFS 获取程序集信息", "log-error");
      return null;
    }
    try {
  ...
  if (dllMode === "Git") {
    if (!dllModeValue) {
      pushLog("未配置 Git 获取程序集信息", "log-error");
      return null;
    }
    try {
  ...
  if (dllMode === "DLL名称") {
    ...
    if (!patterns) {
      pushLog("未配置 DLL名称 模式", "log-error");
      return null;
    }
    const r = await cmdInvoke<string[]>("read_dlls_by_name", {
      dir: clientPath,
      patterns,
    });
    if (r.code !== 0 || !r.data) {
      pushLog(`按 DLL名称 读取失败：${r.data}`, "log-error");
      return null;
    }
    return r.data.map((p) =>
      removeSlash(p).substring(removeSlash(p).lastIndexOf("/") + 1)
    );
  }
  if (dllMode === "当天" || dllMode === "最近3天" || dllMode === "日期范围") {
    ...
    if (startDate && endDate) {
      const r = await cmdInvoke<string[]>("read_dlls_in_date_range", {
        dir: clientPath,
        startDate,
        endDate,
      });
...
const toServerConfigs = async (
  moduleKey: "webApiHost" | "scheduleServer" | "webClient" | "spcMonitor"
): Promise<PublishServerType[] | null> => {
  const ac: any = store.draft.appconfigDraft || {};
  const cfg = ac.configItems?.[moduleKey] as
  ...
  if (cfg.serverArr && cfg.serverArr.length > 0)
    serverArr = cfg.serverArr as SelectServerType[];
  else {
    ...
    if (pid && env) {
      try {
        const r = await appconfigDb.getPublishAppconfigs(
          pid as number,
          env as number
        );
        const full = (r as any)?.data?.data;
        if (full?.configItems?.[moduleKey]?.serverArr?.length) {
          serverArr = full.configItems[moduleKey]
  ...
  if (!serverArr || serverArr.length === 0) {
    const ids = [...(store.draft.serverIds || [])];
    if (ids.length === 0) {
      pushLog(`${moduleKey} 无可用服务器`, "log-error");
      return null;
    }
    ...
    for (const sid of ids) {
      ...
      if (!srv) {
        pushLog(`服务器 #${sid} 不存在，跳过`, "log-warning");
        continue;
      }
      list.push({
        serverName: srv.name,
        ...
        serverConfigs: [
          {
            serverIdentity: srv.name,
            publishPath: cfg.serverPath || `C:/publish/${moduleKey}`,
            publishFiles,
          },
        ],
      });
  ...
  for (const srv of serverArr) {
    ...
    for (const sp of srv.serverPathArr || []) {
      ...
      for (const v of sp.value) {
        if (!v.path) continue;
        serverConfigs.push({
          serverIdentity: String(v.identity || detail.name),
          publishPath: String(v.path),
          publishFiles,
        });
      }
    }
...
const toWpfConfigs = async (): Promise<PublishWpfType[] | null> => {
  ...
  const exists = await cmdInvoke("exists", { path: clientPath });
  if (exists.code !== 0) {
    pushLog(`WpfClient 客户端路径不存在：${clientPath}`, "log-error");
    return null;
  }
  ...
  if (cfg.generateDirJson) {
    try {
      generateDirs = JSON.parse(cfg.generateDirJson) as string[];
    } catch {
      /* ignore */
    }
  }
  ...
  if (cfg.serverArr && cfg.serverArr.length > 0)
    ...
    if (pid && env) {
      try {
        ...
        const full = (r as any)?.data?.data;
        if (full?.configItems?.wpfClient?.serverArr?.length)
          serverArr = full.configItems.wpfClient
            .serverArr as SelectServerType[];
      } catch {
        /* ignore */
      }
  ...
  if (!serverArr || serverArr.length === 0) {
    const ids = [...(store.draft.serverIds || [])];
    if (ids.length === 0) {
      pushLog("WpfClient 无可用服务器", "log-error");
      return null;
    }
  ...
  for (const srv of serverArr) {
    ...
    if (!publishPath) {
      pushLog(
        `WpfClient 服务器 ${srv.name} 未配置发布路径，跳过`,
        "log-warning"
      );
      continue;
    }
...
const toPublishInput = async (): Promise<RemotePublishType | null> => {
  const ac: any = store.draft.appconfigDraft || {};
  const opts = store.draft.publishOptions || {
    isBackup: 0,
    isNewVersion: false,
  };
  if (!store.draft.projectId) {
    pushLog("缺少项目", "log-error");
    ElMessage.warning("请先选择项目");
    return null;
  }
  if (!ac.environment) {
    pushLog("缺少环境（environment）", "log-error");
    ElMessage.warning("请先配置应用配置中的环境");
    return null;
  }
  ...
  const input: RemotePublishType = {
    ...
    isNewVersion:
      (opts.isNewVersion as boolean | null) ??
      ac.configItems?.isNewVersion ??
      false,
    backupBasePath:
      (opts.backupBasePath as string) ||
      (ac.configItems?.backupBasePath as string) ||
      undefined,
    webApiHost,
    webClient,
    scheduleServer,
...
const onPrecheck = async () => {
  if (!store.canPublish) {
    ElMessage.warning("请先完成必选步骤");
    pushLog("预检失败：尚有必选步骤未完成", "log-error");
    return;
  }
  if (!hasAppconfig.value) {
    ElMessage.warning("请先配置应用配置");
    pushLog("预检失败：未配置应用配置", "log-error");
    return;
  }
  ...
  const ac: any = store.draft.appconfigDraft || {};
  pushLog(
    `应用配置: 环境 ${envLabel.value} / ${buildModeLabel.value} / dllMode ${
      ac.dllMode || "—"
    }`
  );
  try {
    ...
    // 校验项目存在
    if (store.draft.projectId) {
      const pr = await projectDb.getProjectById(
        store.draft.projectId as number
      );
      if ((pr as any)?.code !== 0 || !(pr as any)?.data?.data?.id)
        throw new Error("项目不存在或读取失败");
    }
    ...
    // 校验至少一个模块有文件
    const counts = [
      input.webApiHost?.reduce(
        (n, s) =>
          n + s.serverConfigs.reduce((a, c) => a + c.publishFiles.length, 0),
        0
      ) || 0,
      input.scheduleServer?.reduce(
        (n, s) =>
          n + s.serverConfigs.reduce((a, c) => a + c.publishFiles.length, 0),
        0
      ) || 0,
      input.webClient?.reduce(
        (n, s) =>
          n + s.serverConfigs.reduce((a, c) => a + c.publishFiles.length, 0),
        0
      ) || 0,
      input.spcMonitor?.reduce(
        (n, s) =>
          n + s.serverConfigs.reduce((a, c) => a + c.publishFiles.length, 0),
        0
      ) || 0,
      input.wpfClient?.reduce(
        (n, s) => n + s.publishFiles.reduce((a, d) => a + d.files.length, 0),
        0
      ) || 0,
    ];
    const totalFiles = counts.reduce((a, b) => a + b, 0);
    if (totalFiles === 0)
      throw new Error("无待发布文件（请检查客户端路径与 dllMode）");
    ...
    if (input.backupBasePath) {
      ...
      if (r.code !== 0)
        pushLog(
          `备份路径不存在，将在发布时创建：${input.backupBasePath}`,
          "log-warning"
        );
...
const executeRemotePublish = async (input: RemotePublishType) => {
  ...
  if (!ok) {
    pushLog(`创建临时发布目录失败：${mPublishDir}`, "log-error");
    return false;
  }
  ...
  const modules: { key: string; clientPath: string | null }[] = [
    {
      key: "WebApiHost",
      clientPath: (input as any).webApiHost
        ? (store.draft.appconfigDraft as any)?.configItems?.webApiHost
            ?.clientPath || null
        : null,
    },
    {
      key: "ScheduleServer",
      clientPath: (input as any).scheduleServer
        ? (store.draft.appconfigDraft as any)?.configItems?.scheduleServer
            ?.clientPath || null
        : null,
    },
    {
      key: "WebClient",
      clientPath: (input as any).webClient
        ? (store.draft.appconfigDraft as any)?.configItems?.webClient
            ?.clientPath || null
        : null,
    },
    {
      key: "SpcMonitor",
      clientPath: (input as any).spcMonitor
        ? (store.draft.appconfigDraft as any)?.configItems?.spcMonitor
            ?.clientPath || null
        : null,
    },
  ];
  for (const m of modules) {
    ...
    const copyRes = await cmdInvoke("copy_dll_files", {
      source: m.clientPath,
      destination: outPath,
      delDestination: true,
    });
    ...
    if (copyRes.code !== 0) {
      const alt = await cmdInvoke("copy_path", {
        source: m.clientPath,
        destination: outPath,
        ...getRetryArgs("copy"),
      });
      if (alt.code !== 0) {
        pushLog(`复制 ${m.key} 失败：${alt.data || copyRes.data}`, "log-error");
        return false;
      }
    }
  ...
  if (input.wpfClient && input.wpfClient.length > 0) {
    const wpfClientPath = (store.draft.appconfigDraft as any)?.configItems
      ?.wpfClient?.clientPath as string | null;
    const generateDirJson = (store.draft.appconfigDraft as any)?.configItems
      ?.wpfClient?.generateDirJson as string | null;
    if (wpfClientPath) {
      ...
      if (generateDirJson) {
        try {
          ...
          for (const d of dirs) {
            ...
            await createDir(dst);
            const r = await cmdInvoke("copy_dll_files", {
              source: src,
              destination: dst,
              delDestination: true,
            });
            if (r.code !== 0) {
              ...
              if (alt.code !== 0) {
                pushLog(`复制 WpfClient/${d} 失败：${alt.data}`, "log-error");
                return false;
              }
            }
          }
        ...
        const r = await cmdInvoke("copy_dll_files", {
          source: wpfClientPath,
          destination: outWpf,
          delDestination: true,
        });
        if (r.code !== 0)
          pushLog(`复制 WpfClient 失败：${r.data}`, "log-warning");
      }
  ...
  const saveRes = await cmdInvoke("save_content_to_file", {
    content: JSON.stringify(input, null, 2),
    filePath: configPath,
  });
  if (saveRes.code !== 0)
    pushLog(`保存发布配置失败：${saveRes.data}`, "log-warning");

  const remotePublishOne = async (
    servers: PublishServerType[] | null,
    serviceName: string
  ) => {
    ...
    for (const srv of servers) {
      ...
      for (const cfg of srv.serverConfigs) {
        const logPrefix = formatServiceLog(
          srv.serverName || serviceName,
          srv.serverIp,
          serviceName,
          cfg.serverIdentity
        );
        ...
        let stopOk = true;
        if (osName === "Windows") {
          const r = await cmdInvoke("execute_remote_command", {
            username,
            password: rawPwd,
            server: serverAddr,
            command: `net stop "${cfg.serverIdentity}"`,
            ...getRetryArgs("service"),
          });
          if (r.code !== 0) {
            const q = await cmdInvoke("execute_remote_command", {
              username,
              password: rawPwd,
              server: serverAddr,
              command: `sc query "${cfg.serverIdentity}"`,
            });
            stopOk = String(q.data || "").includes("STOPPED");
            if (!stopOk)
              pushLog(`${logPrefix} 停止失败：${r.data}`, "log-error");
          }
          ...
          const r = await cmdInvoke("execute_remote_command", {
            ...
            password: rawPwd,
            server: serverAddr,
            command: `docker stop ${cfg.serverIdentity}`,
          });
          if (r.code !== 0) {
            pushLog(`${logPrefix} 停止失败：${r.data}`, "log-error");
            stopOk = false;
          ...
          pushLog(`未知 OS：${osName}`, "log-error");
          return false;
        }
        ...
        for (const f of cfg.publishFiles) {
          ...
          const candidates = [
            localPath,
            `${removeSlash(mPublishDir)}/WpfClient/${f}`,
            `${removeSlash(mPublishDir)}/${serviceName}/${f}`,
          ];
          let uploaded = false;
          for (const lp of candidates) {
            const up = await cmdInvoke("upload_server_files", {
              localPaths: [lp],
              remotePaths: [`${removeSlash(cfg.publishPath)}/${f}`],
              username,
              password: rawPwd,
              server: serverAddr,
            });
          ...
          if (!uploaded) {
            ...
            const up2 = await cmdInvoke("upload_server_files", {
              localPaths: [`${removeSlash(mPublishDir)}/${serviceName}/${f}`],
              remotePaths: [`${removeSlash(cfg.publishPath)}/${f}`],
              username,
              password: rawPwd,
              server: serverAddr,
            ...
            if (up2.code !== 0) {
              pushLog(`${logPrefix} 上传 ${f} 失败：${up2.data}`, "log-error");
              return false;
            }
          }
        }
        ...
        if (osName === "Windows") {
          const r = await cmdInvoke("execute_remote_command", {
            ...
            password: rawPwd,
            server: serverAddr,
            command: `net start "${cfg.serverIdentity}"`,
            ...getRetryArgs("service"),
          });
          if (r.code !== 0) {
            pushLog(`${logPrefix} 启动失败：${r.data}`, "log-error");
            startOk = false;
          }
        } else if (osName === "Docker") {
          const r = await cmdInvoke("execute_remote_command", {
            ...
            password: rawPwd,
            server: serverAddr,
            command: `docker start ${cfg.serverIdentity}`,
          });
          if (r.code !== 0) {
            pushLog(`${logPrefix} 启动失败：${r.data}`, "log-error");
            startOk = false;
  ...
  if (
    input.webApiHost &&
    !(await remotePublishOne(input.webApiHost, "WebApiHost"))
  )
    return false;
  if (
    input.scheduleServer &&
    !(await remotePublishOne(input.scheduleServer, "ScheduleServer"))
  )
    return false;
  if (
    input.spcMonitor &&
    !(await remotePublishOne(input.spcMonitor, "SpcMonitor"))
  )
    return false;
  if (
    input.webClient &&
    !(await remotePublishOne(input.webClient, "WebClient"))
  )
    return false;
  if (input.wpfClient) {
    for (const wpf of input.wpfClient) {
      ...
      const totalFiles = wpf.publishFiles.reduce(
        (n, d) => n + d.files.length,
        0
      );
      pushLog(
        `WpfClient ${wpf.serverName} (${osName}) 待部署 ${totalFiles} 个文件到 ${wpf.publishPath}`,
        "log-info"
      );
      let wpfOk = true;
      for (const dir of wpf.publishFiles) {
        if (!dir.files || dir.files.length === 0) {
          pushLog(
            `WpfClient ${wpf.serverName} 目录 ${dir.dirName} 无待发布文件，跳过`,
            "log-warning"
          );
          continue;
        }
        pushLog(
          `WpfClient ${wpf.serverName} 正在部署目录 ${dir.dirName} (${dir.files.length} 个文件)...`
        );
        for (const f of dir.files) {
          const localPath = `${removeSlash(mPublishDir)}/WpfClient/${
            dir.dirName
          }/${f}`;
          const fallbackPath = `${removeSlash(mPublishDir)}/WpfClient/${f}`;
          const remotePath = `${removeSlash(wpf.publishPath)}/${
            dir.dirName
          }/${f}`;
          ...
          if (!uploaded) {
            pushLog(
              `WpfClient ${wpf.serverName} 上传 ${dir.dirName}/${f} 失败：${lastErr}`,
              "log-error"
            );
            wpfOk = false;
            break;
          ...
          pushLog(
            `WpfClient ${wpf.serverName} 已部署 ${dir.dirName}/${f}`,
            "log-info"
          );
        }
        if (!wpfOk) break;
...
const onPublish = async () => {
  if (!store.canPublish) {
    ElMessage.warning("请先完成必选步骤");
    pushLog("发布失败：尚有必选步骤未完成", "log-error");
    return;
  }
  if (!hasAppconfig.value) {
    ElMessage.warning("请先配置应用配置");
    pushLog("发布失败：未配置应用配置", "log-error");
    return;
  }
  ...
  try {
    const input = await toPublishInput();
    if (!input) {
      pushLog("发布中止：聚合入参失败", "log-error");
      ElMessage.error("发布失败：请检查日志");
      return;
    }
    pushLog(
      `项目: ${input.projectName} 环境: ${displayEnvironment(
        input.environment
      )} 备份: ${input.isBackup === 1 ? "开启" : "关闭"}`
    );
    ...
    try {
      ...
      const del = await cmdInvoke("delete_paths", { paths: [dir] });
      if (del.code !== 0)
        pushLog(
          `清理临时目录失败：${del.msg || del.data}，请手动清理 ${dir}`,
          "log-warning"
        );
    } catch (e: unknown) {
      ...
      try {
        const dir = await papersPublishDir();
        pushLog(`清理临时目录失败：${msg}，请手动清理 ${dir}`, "log-warning");
      } catch {
        pushLog(`清理临时目录失败：${msg}`, "log-warning");
      }
    }
  }
};
...
.preview-log .log-success {
  color: #67c23a;
}
.preview-log .log-error {
  color: #f56c6c;
}
.preview-log .log-warning {
  color: #e6a23c;
}
.preview-log .log-info {
  color: #e6e6e6;
}
</style>
