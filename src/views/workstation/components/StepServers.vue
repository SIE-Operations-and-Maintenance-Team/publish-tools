<template>
  <div class="step-servers">
    <div style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap">
      <el-button size="small" type="primary" :loading="scanning" @click="onScanLocal">扫描本机</el-button>
      <el-button size="small" type="warning" :loading="scanning" @click="onScanRemote">扫描远端</el-button>
    ...
    <div v-else class="server-cards">
      ...
      >
        <div style="display: flex; gap: 8px; align-items: flex-start">
          <el-checkbox :model-value="selectedIds.includes(srv.id as number)" @change="() => toggleSelected(srv.id as number)" @click.stop />
          <div style="flex: 1; min-width: 0">
            <div style="font-weight: 600; word-break: break-all">{{ srv.name }}</div>
            <div style="font-size: 12px; color: var(--el-text-color-secondary); margin-top: 2px">
              <el-tag size="small" :type="srv.os === 2 ? 'success' : 'info'" style="margin-right: 6px">{{ srv.os === 2 ? 'Docker' : 'Windows' }}</el-tag>
              {{ srv.ip }}:{{ srv.port }} · {{ srv.account || '—' }}
            </div>
            <div v-if="srv.description" style="font-size: 12px; color: var(--el-text-color-secondary); margin-top: 4px; word-break: break-all">{{ srv.description }}</div>
          </div>
        </div>
      </el-card>
    </div>
    ...
    <!-- 远端扫描目标选择 -->
    <el-dialog v-model="remotePickerVisible" title="选择远端服务器" width="420px" :close-on-click-modal="false">
      <el-select v-model="remoteTargetId" placeholder="请选择要扫描的远端服务器" style="width: 100%">
        <el-option v-for="s in serverList" :key="s.id" :label="`${s.name} (${s.ip}:${s.port})`" :value="s.id" />
      </el-select>
      <template #footer>
        <el-button @click="remotePickerVisible = false">取消</el-button>
        <el-button type="primary" :disabled="!remoteTargetId" @click="doScanRemoteWithTarget">开始扫描</el-button>
      </template>
    </el-dialog>

    <!-- 发现结果抽屉 -->
    <el-drawer v-model="drawerVisible" :title="drawerTitle" direction="rtl" size="520px" :close-on-click-modal="false">
      <div v-loading="scanning">
        ...
        <div v-else class="discovery-cards">
          <el-card v-for="(item, idx) in discoveryResults" :key="`${item.serviceName}-${idx}`" shadow="never" style="margin-bottom: 10px">
            <div style="display: flex; gap: 8px; align-items: flex-start">
              <el-checkbox :model-value="selectedDiscoveryIdx.includes(idx)" @change="(v: boolean) => toggleDiscovery(idx, v)" />
              <div style="flex: 1; min-width: 0">
                ...
                <div style="margin-top: 6px; font-size: 12px">
                  <span style="color: var(--el-text-color-secondary)">建议发布目录：</span>
                  ...
                  <el-tag v-if="!item.suggestedPublishPath" type="warning" size="small" style="margin-left: 6px">容器未挂载宿主机目录</el-tag>
                </div>
      ...
      <template #footer>
        <div style="display: flex; justify-content: space-between; align-items: center">
          <span style="font-size: 12px; color: var(--el-text-color-secondary)">已选 {{ selectedDiscoveryIdx.length }} 项</span>
          <div>
            <el-button @click="drawerVisible = false">关闭</el-button>
            <el-button type="primary" :disabled="selectedDiscoveryIdx.length === 0" :loading="importing" @click="onConfirmImport">确认导入</el-button>
          </div>
        </div>
      </template>
    </el-drawer>

    <component :is="ServerDialogComp" ref="serverDialogRef" @refresh="loadServers" />
  </div>
</template>
...
const ServerDialogComp = defineAsyncComponent(() => import('@/views/servers/components/serverDialog.vue'));

const serverDialogRef = ref<any>(null);
const serverList = ref<RowServerType[]>([]);
const selectedIds = ref<number[]>([...(store.draft.serverIds || [])]);

const { scanning, results: discoveryResults, error: discoveryError, loadPrefixes, scanLocal, scanRemote } = useServiceDiscovery();
...
const doScanRemoteWithTarget = async () => {
  ...
  drawerVisible.value = true;
  await loadPrefixes();
  await scanRemote({ ip: `${target.ip}:${target.port}`, account: target.account, pwd: target.pwd });
};

const toggleDiscovery = (idx: number, val: boolean) => {
  if (val) {
    if (!selectedDiscoveryIdx.value.includes(idx)) selectedDiscoveryIdx.value.push(idx);
  } else {
    selectedDiscoveryIdx.value = selectedDiscoveryIdx.value.filter((v) => v !== idx);
  }
};
...
const onConfirmImport = async () => {
  ...
  for (const idx of selectedDiscoveryIdx.value) {
    ...
    const row: RowServerType = {
      ...
      account: '',
      pwd: '',
      description: item.suggestedPublishPath ? `建议发布目录: ${item.suggestedPublishPath}` : `WorkingDir: ${item.rawPath || ''}`,
    } as RowServerType;
  ...
  if (success > 0) {
    ...
    for (const id of newIds) if (!selectedIds.value.includes(id)) selectedIds.value.push(id);
    store.draft.serverIds = [...selectedIds.value];
    store.persist();
  }
...
watch(
  ...
  (v) => {
    ...
    if (JSON.stringify(sorted) !== JSON.stringify(curr)) selectedIds.value = [...(v || [])];
  }
);
